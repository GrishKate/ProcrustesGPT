...
def replace_modules(
        ...
        new_module_factory,  #: Callable[[AnyModule, Optional[int]],Module,],
        replace_layers: bool,
) -> None:
...
class NewHead(nn.Module):
    def __init__(self, hid_dim, emb_dim, dtype):
        super().__init__()
        self.dtype = dtype
        self.mean = None
        self.Q = None
    ...
    def forward(self, x):
        ...
        if self.mean is not None:
            x = self.lm_head(x) + x @ self.e @ self.mean
        return x


def fuse_modules(model_adapter: ModelAdapter, detach_head=True, model_name='opt') -> None:
    """
    ...
    for W in model_adapter.get_embeddings():
        ...
        mean_emb = W_.mean(dim=-1, keepdim=True) if mean_emb is None else mean_emb
    ...
    dif = torch.norm(model_adapter.get_lm_head().weight.data - model_adapter.get_embeddings()[0].weight.data)
    if detach_head or dif > 1e-2:
        fuse_ln_linear(model_adapter.get_pre_head_layernorm(), [model_adapter.get_lm_head()])
    else:
        head = model_adapter.get_lm_head().weight.data
        layernorm = model_adapter.get_pre_head_layernorm()
        out, inp = head.shape
        layer = NewHead(inp, out, head.dtype)
        layer.lm_head = model_adapter.get_lm_head()
        layer.Q = layernorm.weight
        if hasattr(layernorm, 'bias'):
            layer.b = layernorm.bias
        if mean_emb is not None:
            layer.mean = nn.Parameter(mean_emb.T)
            layer.e = nn.Parameter(torch.ones(inp, 1, dtype=head.dtype))
        model_adapter.model.lm_head = layer
