...
class CompressedLlamaDecoderLayer(LlamaDecoderLayer):
    ...
    def forward(
            ...
            attention_mask=None,  #: Tensor | None = None,
            position_ids=None,  #: LongTensor | None = None,
            past_key_value=None,  #: tuple[Tensor] | None = None,
            output_attentions=False,  #: bool | None = False,
            use_cache=False,  #: bool | None = False,
            **kwargs,
    ) -> tuple:
        """
        Args:
        ...
        if self.attn_shortcut_Q is not None:
            rotated_residual = residual @ self.attn_shortcut_Q
        ...
        if self.mlp_shortcut_Q is not None:
            rotated_residual = residual @ self.mlp_shortcut_Q
            hidden_states = rotated_residual + hidden_states
        else:
            hidden_states = residual + hidden_states
...
class LlamaModelAdapter(ModelAdapter):
    ...
    def convert_layer_to_compressed(self, layer: Module, layer_idx) -> Module:
    ...
    def _from_pretrained(
            ...
            *,
            ...
            token=None,  #: str | bool | None = None,
    ):  # -> ModelAdapter | None:
        if not (model_name.startswith("meta-llama/Llama-2") or model_name.startswith("meta-llama/Meta-Llama-3")):
            return None

        model = LlamaForCausalLM.from_pretrained(
            model_path, torch_dtype=dtype, local_files_only=local_files_only,# token=token,
        )
